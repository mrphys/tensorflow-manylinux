...
# https://github.com/pypa/manylinux FROM quay.io/pypa/manylinux_2_28_x86_64 
...
# Toolchain: GCC 14 # x86_64 image: quay.io/pypa/manylinux_2_28_x86_64 
...
#was CUDA_VERSION=12.5 

ARG CUDA_VERSION=13.0 
ARG CUDNN_VERSION=9.3 
ARG PY_VERSIONS="3.9 3.10 3.11 3.12 3.13 3.14 3.15" 
...
RUN for PYVER in ${PY_VERSIONS}; 
    do ${PYBIN}${PYVER} -m pip install tensorflow==${TF_VERSION}; 
    done 

# ------------------------------------------------- 
# FFTW3 (build from source) 
# ------------------------------------------------- 
ARG PREFIX=/dt9/usr # was 3.3.9 RUN cd /opt && \
...
# Spiral waveform 
# ------------------------------------------------- 
RUN cd /opt && \ git clone https://github.com/mrphys/spiral-waveform --branch v1.0.0 && \ 
    cd spiral-waveform && \ make install INSTALL_PREFIX=${PREFIX} 
...
# Patch auditwheel 
# ------------------------------------------------- 
# Install auditwheel for all Python versions RUN for pybin in /opt/python/*/bin/python*; 
...
RUN for py in /opt/python/*/lib/python3.*; 
